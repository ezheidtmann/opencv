...
class Builder:
    ...
    def _build(self, outdir):
        ...
        for t in self.targets:
            ...
            dirs.append(mainBD)
            cmake_flags = []
            # cmake_flags.append("-DBUILD_CUDA_STUBS=OFF")
            cmake_flags.append("-DBUILD_DOCS=OFF")
            cmake_flags.append("-DBUILD_EXAMPLES=OFF")
            cmake_flags.append("-DBUILD_JASPER=OFF")
            cmake_flags.append("-DBUILD_JPEG=OFF")
            cmake_flags.append("-DBUILD_OPENEXR=OFF")
            cmake_flags.append("-DBUILD_PACKAGE=OFF")
            cmake_flags.append("-DBUILD_PNG=OFF")
            cmake_flags.append("-DWITH_PNG=OFF")
            # cmake_flags.append("-DBUILD_SHARED_LIBS=ON")
            cmake_flags.append("-DBUILD_TBB=OFF")
            cmake_flags.append("-DBUILD_TIFF=OFF")
            # cmake_flags.append("-DBUILD_WITH_DEBUG_INFO=OFF")
            # cmake_flags.append("-DBUILD_WITH_DYNAMIC_IPP=OFF")
            # cmake_flags.append("-DBUILD_ZLIB=ON")
            # cmake_flags.append("-DBUILD_opencv_apps=ON")
            cmake_flags.append("-DBUILD_opencv_calib3d=OFF")
            # cmake_flags.append("-DBUILD_opencv_core=ON")
            cmake_flags.append("-DBUILD_opencv_features2d=OFF")
            cmake_flags.append("-DBUILD_opencv_flann=OFF")
            cmake_flags.append("-DBUILD_opencv_highgui=OFF")
            cmake_flags.append("-DBUILD_opencv_imgcodecs=OFF")
            cmake_flags.append("-DBUILD_opencv_imgproc=OFF")
            cmake_flags.append("-DBUILD_opencv_ml=ON")
            cmake_flags.append("-DBUILD_opencv_objdetect=OFF")
            cmake_flags.append("-DBUILD_opencv_photo=OFF")
            cmake_flags.append("-DBUILD_opencv_shape=OFF")
            cmake_flags.append("-DBUILD_opencv_stitching=OFF")
            cmake_flags.append("-DBUILD_opencv_superres=OFF")
            cmake_flags.append("-DBUILD_opencv_ts=OFF")
            cmake_flags.append("-DBUILD_opencv_video=OFF")
            cmake_flags.append("-DBUILD_opencv_videoio=OFF")
            cmake_flags.append("-DBUILD_opencv_videostab=OFF")
